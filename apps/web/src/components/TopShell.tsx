...
export function TopShell({ children }: { children: React.ReactNode }) {
  ...
  const currentItem = NAV.find(n => pathname?.startsWith(n.href) && n.href !== '/dashboard')
          ...
          <div className="hidden md:flex gap-1">
            {quickTabItems.slice(0, 5).map(tab => (
              <Link key={tab.href} href={tab.href}
                className={`px-2.5 py-1 text-xs font-medium rounded-md transition-colors ${
                  pathname?.startsWith(tab.href) ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-100'
                }`}>
                {tab.label}
              </Link>
            ))}
