...
export class UnitsService {
  ...
  async findOne(orgId: string, id: string): Promise<UnitResponseDto> {
    const item = await this.prisma.unitOfMeasure.findFirst({ where: { id, orgId, deletedAt: null } });
  ...
  async create(orgId: string, dto: CreateUnitDto): Promise<UnitResponseDto> {
    const anyExisting = await this.prisma.unitOfMeasure.findFirst({ where: { orgId, shortName: dto.shortName } });
    if (anyExisting) {
      if (!anyExisting.deletedAt) throw new ConflictException('Одиниця з такою скороченою назвою вже існує');
      const restored = await this.prisma.unitOfMeasure.update({ where: { id: anyExisting.id }, data: { ...dto, deletedAt: null } });
  ...
  async update(orgId: string, id: string, dto: UpdateUnitDto): Promise<UnitResponseDto> {
    ...
    if (dto.shortName && existing.shortName !== dto.shortName) {
      const duplicate = await this.prisma.unitOfMeasure.findFirst({ where: { orgId, shortName: dto.shortName, deletedAt: null, NOT: { id } } });
  ...
  async remove(orgId: string, id: string): Promise<void> {
    const existing = await this.prisma.unitOfMeasure.findFirst({ where: { id, orgId, deletedAt: null } });
    if (!existing) throw new NotFoundException('Одиниця виміру не знайдена');
    await this.prisma.unitOfMeasure.update({ where: { id, orgId }, data: { deletedAt: new Date() } });
    await this.cache.del(cacheKey(orgId));
  }
  ...
  toDto(item: {
    ...
    return {
      ...
      shortName: item.shortName, isSystem: item.isSystem, coefficient: item.coefficient,
