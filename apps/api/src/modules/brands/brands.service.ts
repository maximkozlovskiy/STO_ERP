...
export class BrandsService {
  ...
  async findAll(orgId: string): Promise<{ items: BrandResponseDto[]; total: number }> {
    const cached = await this.cache.get<{ items: BrandResponseDto[]; total: number }>(cacheKey(orgId));
    if (cached) return cached;

    const [items, total] = await this.prisma.$transaction([
      this.prisma.brand.findMany({ where: { orgId, deletedAt: null }, orderBy: { name: 'asc' }, take: 1000 }),
  ...
  async create(orgId: string, dto: CreateBrandDto): Promise<BrandResponseDto> {
    ...
    if (anyExisting) {
      ...
      const restored = await this.prisma.brand.update({ where: { id: anyExisting.id }, data: { ...dto, deletedAt: null } });
  ...
  async update(orgId: string, id: string, dto: UpdateBrandDto): Promise<BrandResponseDto> {
    ...
    const duplicate = await this.prisma.brand.findFirst({ where: { orgId, name: dto.name, NOT: { id }, deletedAt: null } });
  ...
  private toDto(item: { id: string; orgId: string; name: string; createdAt: Date; updatedAt: Date }): BrandResponseDto {
    return { id: item.id, orgId: item.orgId, name: item.name, createdAt: item.createdAt, updatedAt: item.updatedAt };
  }
}
