...
describe('PurchaseOrders — HTTP Contract', () => {
  ...
  describe('GET /purchase-orders/:id/linked-documents', () => {
    it('повертає 200 і делегує (orgId, id)', async () => {
      ...
      const res = await (app as NestFastifyApplication).inject({
        method: 'GET',
        url: `/purchase-orders/${VALID_UUID}/linked-documents`,
      });
      expect(res.statusCode).toBe(200);
    ...
    it('повертає 400 для не-UUID id', async () => {
      const res = await (app as NestFastifyApplication).inject({
        method: 'GET',
        url: '/purchase-orders/not-uuid/linked-documents',
      });
      expect(res.statusCode).toBe(400);
    });
