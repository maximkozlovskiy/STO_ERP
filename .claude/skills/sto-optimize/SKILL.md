...
**Сигнал:** modal/form з таблицею редагування рядків (lines/parts/items) має `<tfoot>` який рендерить підсумок VAT + total. Розробник пише два окремих `lines.reduce(...)` блоки (один для VAT, один для total) у JSX напряму, обидва витягують ті ж самі поля через `toNumberOrUndefined(l.normoHours)` + `toNumberOrUndefined(l.price)`. Кожен render батька (typing у будь-якому полі форми) → обидва reduce проходять весь масив × кожен row парситься 2-4 рази (string→Number). При 20 рядках × ~5 keystrokes за save click → 200 string conversions. Особливо помітно бо: VAT-колонка conditional `vatMode !== 'NONE'`, тож twin-scan легко "не помітити" — другий reduce візуально окремий.

**Причина виникнення:** JSX-driven код пишеться лінійно: «ось VAT-cell, ось reduce для нього; ось total-cell, ось reduce для нього». Обертати усе у useMemo здається надмірним — це просто sum, два значення. Розробник не помічає що `toNumberOrUndefined` (валідація string→Number з comma→dot) НЕ дешева — це Regex + Number construction + isFinite check. Помножити на N рядків × 2 reduce проходів × M ререндерів. Інший заплутувач: VAT може бути 0 (`vatRate === 0`) — тоді VAT reduce безкорисний, але все одно проходить.

**Підхід до виявлення:** grep `\.reduce\(.*\n.*\.reduce\(` (multiline) у `.tsx` файлах форм з таблицями. Також: `<tfoot>.*reduce` — будь-який total-row у footer. Cross-check: чи два reduce ходять одним і тим же масивом? Чи extracting'ять ті ж самі поля (normoHours/price, quantity/price)? Якщо так — кандидат. Особливий signal: `vatMode !== 'NONE'` як conditional навколо одного reduce — це типовий twin-scan з VAT-toggle.

**Підхід до фіксу:** один `useMemo(() => { let total = 0; let vat = 0; for (const l of lines) { const h = toNumberOrUndefined(...); ...; if (vatRate > 0) vat += sum * vatRate / 100; } return { total, vat }; }, [lines, vatRate])`. У JSX: `{linesTotals.total.toFixed(2)}` + `{linesTotals.vat.toFixed(2)}`. Один pass через масив, одна string→Number конверсія per row. Не плутати з: (a) `reduce` для різних масивів (lines vs parts — там вони обидва потрібні, але кожен у свій useMemo); (b) `reduce` що computes різну агрегацію (sum vs avg vs max — там single pass з кількома акумуляторами все одно працює).

**Реальний impact:** для WO modal з 20 lines + 15 parts × typing у Опис (10 keystrokes) — раніше: 4 reduce проходів × 35 рядків × 2 `toNumberOrUndefined` calls × 10 keystrokes = 2800 string parsings. Після фіксу: useMemo пропускає при незмінному `lines/parts/vatRate` → 0 reduce проходів на typing у не-table полях; коли lines дійсно змінюється — 2 reduce passes (lines + parts), 1 string parsing per cell. Latency saving залежна від N: ~5-15ms при 20 рядках на mid-range laptop, помітніше на планшеті механіка.

**Де шукати ще:** будь-яка table-edit modal/form з footer-totals: invoice/PO/SD/quote line editors, completion-act line editor, expense-claim items. Conditional VAT-row — типове джерело: бухгалтер додає VAT-mode → програмний нюанс — другий reduce. У dashboard/reports з multiple summary cards над одним масивом (revenue + count + avg) — той самий патерн на 3+ reduce проходи.

---

### 2026-06-11 — Per-row `array.find(x => x.id === row.foreignKey)` у `.map()` рендерах і onChange handlers — O(N×M) → useMemo Map.get O(1)

**Сигнал:** компонент має reference-data масиви (employees/warehouses/units/branches/lifts/vehicles) у стейті або prop, і у `lines.map(line => { const emp = employees.find(e => e.id === line.employeeId); ... })` робить linear scan довідника per row. При 30 рядків × 10 employees × ререндери з частим typing → 300+ find ops per keystroke. Зустрічається також у `onChange={e => { const u = units.find(u => u.id === e.target.value); setForm(...); }}` — кожен select зміна = повний linear scan довідника. У IIFE summary-chip (`{form.vehicleId && (() => { const v = vehicles.find(...); return <span>...</span>; })()}`) — теж linear scan, але conditional.

**Причина виникнення:** `.find()` — ідіоматичний JS, легко читається («знайди той що з потрібним id»). Розробник не думає про асимптотику бо довідники "маленькі" (10-50 елементів). Але це helper, виклик per row × per render → multiplied effect. useMemo Map здається over-engineering для 10 елементів. Окремо: рефакторинг табличних компонентів часто додає `.find()` для нової FK без перегляду цілого file патерну — кожна нова relation = +1 find call per row.

**Підхід до виявлення:** grep `\.find\(.*=>.*\.id\s*===` у `.tsx` файлах modal/form/list. Якщо знайдено всередині `.map()` коли parent re-renders часто (typing у inputs, search debounce, filter changes) → кандидат. Cross-check: чи довідник стабільний (з ref-cache / useQuery з staleTime)? Якщо так — Map можна побудувати раз на mount, переюзати скрізь. Окремий сигнал: ту саму find-pattern бачимо у onChange handlers select-ів — рефакторити одразу обидві.

**Підхід до фіксу:** для кожного reference-data масиву — `const xById = useMemo(() => { const m = new Map<string, X>(); for (const x of xArray) m.set(x.id, x); return m; }, [xArray])`. У map / onChange: `xById.get(rowFk)` — O(1). Build cost: O(N) на mount + при reload довідника. Recoupment break-even — 1-2 render-cycles. Не плутати з: (a) find з compound condition (`find(x => x.fooId === y && x.barId === z)` — там Map по двух ключах громіздкий, краще keep find); (b) пошук по name (UI search) — там contains не справа Map; (c) single find поза `.map()`/`.onChange` — там useMemo Map зайвий.

**Реальний impact:** на WO modal з 15 lines + 10 parts + typing у будь-якому полі (опис, нормогодини, ціна) — раніше: 15 employees.find + 10 warehouses.find + IIFE 3 find (vehicle/lift/branch) = 28 linear scans per render × N keystrokes. Після фіксу: 0 scans (Map.get O(1)). Якщо довідник з 200 елементів (мала org з 200 employees доступний на planшеті) — economy величезна: ~3000 операцій → 28. Maps персистентні через рендери — Garbage Collector тригериться рідше. UX-effect: typing latency у формі знижується на planшетах з низькою CPU — вісібельно швидше editing у real-time.

**Де шукати ще:** будь-який list/table-edit з reference data, що рендериться поруч з form inputs: WO/invoice/PO/SD line editors, dashboard widget що мапить orderRows + lookups, reports table з drill-down lookups. Особливо часто: counterparty/vehicle/lift/branch/warehouse/employee/category resolution у row-rendering. Cross-cutting: якщо реф-дата вже live у `useQuery` з gcTime → персистентний Map дає stable identity для memo-children.
...
**Причина виникнення:** історична: коли transition() писалась перший раз, `wo` об'єкт з child-rel передавався у helper як аргумент (`helper(orgId, wo, userId)`). Потім helpers рефакторингу winесли — кожен став робити свій fresh fetch всередині tx (правильно, бо tx-context дає свіжі дані consistency-wise). Outer include лишився "по інерції" — ніхто не помітив, що child-rel більше не передається у helpers і не використовується у решті тіла. Code review проходить бо include виглядає як "повне завантаження сутності для роботи з нею" — стандартний паттерн. Не помічається бо: (1) функціонально нічого не зламано; (2) profiling row-counts не очевидний у DevTools (видно лише агрегат transition latency); (3) Postgres LIMIT 1000 виконується швидко на indexed FK, тож increment latency лише +5-10ms — у шумі.

**Підхід до виявлення:** для кожного методу з `findFirst.*include.*take:` у `*.service.ts` — прочитати все тіло функції до `return`. Знайти всі читання `wo.XxxxRel` (де XxxxRel — назва relation у include). Якщо ЖОДНОГО — overfetch. Cross-check: усі downstream helpers що приймають `wo` — перевірити їх сигнатури. Якщо helper читає лише `wo.id` + scalar fields, child-rel у helper не потрібен. Окремий сигнал: helpers що мають у тілі `db.X.findMany({ where: { parentId: wo.id, ... } })` — це підтверджує що child-rel вони беруть свіжі з tx-context, а outer include — мертве.

**Підхід до фіксу:** замінити `include: {childRel}` на `select: {...scalar fields actually used...}`. Перерахувати точний набір полів через grep `wo\.` у тілі функції. Залишити лише ID/scalar/FK що читаються. Helper-сигнатури перевірити: якщо helper accepts `{ id, counterpartyId, totalAmount }` shape — outer select cover'ить. Якщо helper accepts ширший shape — або narrow helper, або додати поля у select. Не плутати з: (a) findFirst що повертає row з child-rel у toDto — там include обов'язковий; (b) findOne для UI/API response — там child-rel використовується у response payload (не overfetch).

**Реальний impact:** для transition() — раніше 9 переходів FSM × per-transition include {parts: take:1000} → нуль після фіксу (виключно scalar fields). На work-orders з ~50 parts типове — read 50 рядків × 9 переходів × N users/day. Знято до тисяч непотрібних row marshalling операцій. Найпомітніше у end-of-day workflow коли механіки масово завершують наряди (COMPLETED) — кожен transition тепер лише читає scalar fields замість тягнути всю BOM. Latency reduction per transition: 5-15ms на середніх WO; agregated за зміну — секунди. Memory pressure на Node-side зменшується (RowMaterializer не виділяє об'єкти для unused rows).

**Де шукати ще:** будь-який \*.service.ts метод з patternom `findFirst({include:{childRel:{take:N}}})` + downstream helpers що роблять свої fetches з child-rel. Особливо часті місця: WorkOrders.transition, Invoices.refresh, PurchaseOrders.receive, StockDocuments.post, Settlements.applyTransaction. При додаванні нового transition/post/refresh методу — перевіряти що child-rel у include справді читається у тілі (НЕ у downstream helpers, які роблять свої tx-reads).
...
**Сигнал:** list-page має константу типу `STATUS_TABS = [...]` (10-20 елементів) і у JSX рендерить через `.map(([v, l]) => <button onClick={() => { setX(v); resetPage(); ... }} className={cn(...)}>{l}</button>)` — багаторядковий inline-arrow на кожен item, можливо обгорнутий у `<Tooltip>`. На КОЖЕН render батька (типове джерело — typing у search input через `onChange={e => setSearch(e.target.value)}` що тригерить useQuery) інлайн-arrow створюється N разів, Tooltip-діти reconciler'яться N разів. Зі статичним переліком (status pills, view-mode chips, sort selectors) це чистий waste — items не змінюються, лише active-flag. `memo` неможливий бо inline JSX все одно нова "tree subtree" з кожним render.

**Причина виникнення:** історично list pages пишуться лінійно — натхненним стилем JSX-у з inline-handlers ("ось pills, ось як вони реагують на клік"). Виокремлення кожного типу pills у окремий компонент здається over-engineering для 10 кнопок. Tooltip-обгортка ще більше натхнює inline-arrow паттерн ("тут є умовна логіка") бо conditional render ускладнює факторинг. Не помічається бо: (1) profiler list-page виглядає як один великий "render" — pills tucked у глибині; (2) typing-impact видно лише на slow devices (планшет механіка); (3) statici pill набори часто додаються постпонно (один за одним, FSM-розширення), розробник не повертається до factoring.

**Підхід до виявлення:** у file containing `(STATUS|TYPE|VIEW|MODE|TAB)_(TABS|FILTERS|CHIPS)\s*=\s*\[` (constants для repeater pills) — перевірити чи `.map()` рендерить inline-arrow handler (`onClick={() => ... }`) ВСЕРЕДИНІ JSX. Якщо так — кандидат на extract+memo. Cross-check: чи parent re-renders часто (search/debounce/useQuery refetch)? Якщо так — impact значний. Окремий сигнал: Tooltip обгортка `{STATUS_DESCRIPTIONS[v] && <Tooltip>...</Tooltip>}` — Tooltip + button child reconciler'иться на кожен render батька.

**Підхід до фіксу:** виокремити module-level memo-component `const StatusPill = memo(function StatusPill({ value, label, active, description, onSelect }: Props) { return description ? <Tooltip>{btn}</Tooltip> : btn })`. Props приймати тільки примітиви + ОДИН stable callback (`onSelect`). У parent: `const handleSelect = useCallback((v) => { setX(v); resetPage(); ... }, [resetPage])`. Memo skip коли parent re-render'ить через зміну search/query — props всі stable references, memo пропускає reconcile. Зробити це треба для ВСІХ статичних repeater-pill-наборів сторінки (часто є кілька: status + priority + category). НЕ memo'ити динамічні pills (chips що генеруються з даних — там props всі змінні, memo не допоможе).

**Реальний impact:** для work-orders page з 11 status pills + typing у search input (5-10 keystrokes за пошук) — раніше 11 × 5 = 55 нових onClick closures + 55 Tooltip re-evaluations. Після фіксу — 0 reconcile у pills row. На midrange laptop ~3-5ms total per type session; на планшеті механіка (Bug-prone path) ~15-20ms. Кумулятивно за робочий день — секунди saved + менше GC pressure. Найпомітніше у sessions з частим filtering (адмін що моніторить кілька статусів послідовно).

**Де шукати ще:** усі list-pages з static repeater pills/chips/tabs — work-orders, invoices, purchase-orders, stock-documents, payments, calendar (view-mode), reports (period chips). Особливо часто: FSM-status filters, view-mode switchers, time-period chips, role/type filters. При додаванні нового list-page — стандарт: статичний pill набір → виокремити у `*Pill`/`*Chip` memo component поряд з page-файлом (не у глобальній бібліотеці бо стиль/UX часто page-specific).
...
**Сигнал:** у JSX батьківського компонента (list-page, dashboard, settings) є блок `{(() => { const buildTabs = (item) => [...]; return <Panel tabs={selectedItem ? buildTabs(selectedItem) : undefined} /> })()` — самовиклика��ча функція що тримає локальний хелпер + одразу його виконує. Хелпер тримає bizness-логіку побудови tabs (Badge-форматтери для status/priority, span для overdue, conditional rendering) + декларацію JSX-tree. Кожен render батька → IIFE виконується → `buildTabs` створюється наново → `buildTabs(selectedItem)` обчислюється → новий array з новими React elements (Badge/PanelField/Button з inline onClick). Panel компонент бачить новий `tabs` prop reference → re-render → всі діти reconcile. `tabs` контент сам по собі стабільний (залежить лише від `selectedItem` + `panelConfig` + `nowMs`), але паттерн IIFE forced eager evaluation на кожен render.

**Причина виникнення:** IIFE — типовий "in-place factoring" для двох-трьох кроків що читаються лінійно ("ось billiar, ось виклик, ось JSX"). Розробник інтуїтивно уникає `useMemo` бо: (1) внутрішня функція `buildTabs` має параметр (`item`), `useMemo` повертає лише значення — здається що "не підходить"; (2) JSX всередині використовує decorator callbacks (`{...status: v => <Badge.../>}`) — виглядає як runtime-only логіка; (3) IIFE приховує що це "просто значення" — мозок читає його як "функціональний блок". Не помічається у DevTools бо tabs зазвичай simple JSX — re-render Badge/PanelField не катастрофічний (small subtree).

**Підхід до виявлення:** grep `\{\(\(\) =>` у `apps/web/src/app/**/*.tsx` (стрілкова IIFE) АБО `\{\(function`. Для кожного хіта перевірити: (1) чи всередині є аргумент-функція, що викликається у return (звичайний "build + use" патерн); (2) чи depends-list зрозумілий (всі вхідні значення — `useState`/derived з stable refs); (3) чи зовнішній JSX-prop сприйме reference-equality (типово useMemo helps). Якщо так — кандидат на refactor. Кросс-сигнал: IIFE передає parameter у inner-функцію — зазвичай ця параметер це `selectedItem`/`row`/`subject` що береться з parent state.

**Підхід до фіксу:** двофазний refactor: (1) Lift inner function body у `useMemo(() => { if (!selectedItem) return undefined; const item = selectedItem; return [...tabs...]; }, [selectedItem, ...deps])`. Параметер функції стає змінною з useMemo body — оскільки deps capture parent-state, useMemo гарантує stable reference. (2) Replace inline handlers у tabs JSX на stable references — наприклад `<Button onClick={() => setEditId(item.id)}>` стає `<Button onClick={handleOpenEdit}>` + `useCallback((id: string) => setEditId(id), [])` АБО `<Button onClick={openEditCb}>` де `openEditCb = useCallback(() => setEditId(item.id), [item.id])`. (3) Усі inline `() => Component(value)`-формат decorators у `buildPanelFields(...)` можна лишити inline бо вони викликаються синхронно з useMemo body, не у render-path React елементів. Не плутати з: (a) IIFE для conditional render (`{ open && (() => { ... })() }`) — там можна replace на простий `{open && <X>...</X>}`; (b) IIFE для scope (вирішення name collision) — там useMemo не допоможе.

**Реальний impact:** для list-page з DetailPanel (work-orders, invoices, counterparties detail) — кожен typing у search input або filter change викликав IIFE → buildTabs → новий array of React elements → DetailPanel re-render. Після useMemo — tabs стабільні поки `selectedWO/panelConfig/nowMs` не змінюються. На work-orders зокрема: typing 5 keystrokes у search → 5 непотрібних DetailPanel re-renders → 5 Badge/PanelField reconcile-cycles. Після фіксу — 0. На планшеті механіка з detail panel completedly visible — особливо помітно.

**Де шукати ще:** будь-яка list-page з DetailPanel/Drawer/RightPanel/SidePanel що використовує IIFE для побудови tabs/content. Часто: work-orders/invoices/purchase-orders/counterparties detail panels, dashboard widgets composer, settings page tabs. При додаванні нового list-page з detail panel — стандарт: tabs/content через useMemo, ніколи через IIFE. Cross-check на нові feature: якщо PR додає `(() => { ... })()` у JSX — review має наполягти на useMemo.
...
**Сигнал:** UI компонент рендерить попередження/підсумок з derived string з масиву об'єктів: `{calConflict.conflictSlots.length} слотом(и) {calConflict.conflictSlots.some(s => s.workOrderNumber) && (<> ({calConflict.conflictSlots.filter(s => s.workOrderNumber).map(s => s.workOrderNumber).join(', ')}) </>)}`. Логіка: показати кому належать конфліктні слоти, але тільки якщо є хоч один з workOrderNumber. Twin-scan: `.some()` проходить масив раз для check, `.filter()` проходить раз для filter, `.map()` для extract, `.join()` для concat. 3 intermediate arrays + 4 passes. На КОЖЕН render батька (typing у time-input → setConflict() оновлюється) це повторюється. Масив маленький (1-10 conflict slots), тому micro-impact, але паттерн копіюється на кілька render sites + повторюється у sibling-модалках (WO modal + Calendar modal).

**Причина виникнення:** JSX-style "expressive chains" виглядає декларативно ("ось is-any, ось extract+join"). Розробник свідомо не виокремлює змінну бо інакше треба ділити JSX на дві частини (declaration + use), а inline chain читається як одне речення. Перформанс не очевидний — `.some/.filter/.map/.join` усі O(N), N мале → "оптимізація не потрібна". Не помічається бо profiler не виокремлює рядкові операції з reconcile-фази. Найбільш помітне коли той самий чанк копіюється у sibling-компоненти (DRY violation amplifies impact).

**Підхід до виявлення:** grep `\.some\(.*=>.*\)\s*(&&|\?)` у `apps/web/src/`. Для кожного хіта перевірити чи поряд є `.filter` чи `.map` на тому ж масиві з тією ж predicate. Якщо так — twin-scan. Cross-sigнал: predicates симетричні (`.some(s => s.X)` + `.filter(s => s.X)` — те саме поле/перевірка). Окремо: `.some()` всередині JSX-conditional (`{flag && (...)}`) часто супроводжується той самий predicate у `.filter()` нижче.

**Підхід до фіксу:** useMemo single-pass: `const derived = useMemo(() => { const result: string[] = []; for (const s of array ?? []) { if (s.predicate) result.push(s.field); } return result.join(', '); }, [array])`. Деps — тільки масив (predicate stable). У JSX: `{derived && <> ({derived})</>}`. Один pass + один intermediate array + один join. Якщо derived використовується у двох render-сайтах (banner + confirm modal message) — useMemo гарантує стабільне посилання, ne computed twice per render. Не плутати з: (a) Array methods де predicates differ (`.some(s => s.A)` + `.filter(s => s.B)`) — там useMemo не виграш, треба inline; (b) Short circuits де `.some()` має side effect (rare) — не оптимізувати.

**Реальний impact:** для WO Modal + Calendar Modal — 2 components × 2 render sites × twin-scan на typing у час-input (debounce кожні 400ms). Раніше: typing 5 keystrokes → 10 twin-scans. Після: 5 single-passes. Micro-impact (~0.1-0.5ms per scan), але patтерн копіюється у 2+ файли — DRY single useMemo легше maintain. Бонус: derived string доступний у confirm-dialog message (раніше треба було копіювати the same chain inline).

**Де шукати ще:** будь-який UI з warning/summary banners over array-data: conflict slots (calendar), batch operation results (toast.warning "X з Y added, Y-X failed"), validation summaries (`.some(r => r.error)` + `.filter(r => r.error).map(...)`), linked-docs counters. Особливо часто у twin-modal scenarios де identical логіка copy-paste'нута у WO/Calendar/Invoice modal. При sweep — об'єднати useMemo + extract у custom hook якщо patтерн копіюється в 3+ місцях.
...
- ✅ CreateWorkOrderModal: `linesTotals`/`partsTotals` useMemo з single-pass {total, vat} замість twin-scan `lines.reduce()`+`lines.reduce()` у tfoot (2 проходи × N рядків × toNumberOrUndefined string→Number); `employeesById`/`warehousesById`/`unitsById`/`vehiclesById`/`liftsById`/`branchesById` useMemo Maps замість `.find()` по реф-довідниках у `.map()` рядків і у onChange handlers — O(N×M) → O(N+M build) + O(1) lookup. IIFE summary-chips (`vehicles/lifts/branches.find()` у headerCollapsed) замінено на Map.get-based IIFE для conditional null
