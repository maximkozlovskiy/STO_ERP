...
### 2026-06-11 — Shared FE-BE константа expose-ana з FE side, але backend все ще has inline literals (Bug #432) — backend / single-source-of-truth / FE-BE drift

**Сигнал:** commit що додає shared constant у `packages/shared/src/constants/*.ts` (наприклад `WO_INVOICEABLE_STATUSES`, `WO_SHAREABLE_STATUSES`, `PO_RECEIPT_STATUSES`) і оновлює FE consumers (`apps/web/src/.../*.tsx`) щоб використати її, АЛЕ парний backend service-файл лишається з inline literal-arrays (`['COMPLETED', 'INVOICED']`, `'SENT' | 'PAID'`). Backend gate що мав би бути ТИМ-ЖЕ-whitelist дублюється у 1-3+ місцях inline. Grep підтверджує:

```bash
# Для кожної shared константи у packages/shared виявити backend inline копії:
grep -rn "'COMPLETED', 'INVOICED'" apps/api/src --include="*.ts" | grep -v spec  # шукаємо літерали, що зараз є константою
```

**Причина виникнення:** FE refactor PR натхненний UX-потребою (frontend має кнопку, гейтить FSM-whitelist) — розробник створює shared константу у `@sto/shared` саме щоб FE не дублював значення. Логічно сприймає це як «frontend cleanup» і не зачіпає backend (бо backend тести зелені, нічого не зламано). Backend service зі своєю inline копією continues to work, бо whitelist випадково однаковий. Drift accumulates: shared константа стає «джерелом для FE only», backend — окреме незалежне джерело.

**Підхід до виявлення:** перевіряти **кожен** PR/commit що додає constant у `packages/shared/src/constants/*.ts`:

1. `git diff HEAD~N HEAD -- "packages/shared/src/constants/*.ts" | grep -E "^\+export const ([A-Z_]+)"` → список нових імен.
2. Для кожного імені шукати backend equivalent: `grep -rnE "'(<literal1>)', '(<literal2>)'" apps/api/src --include="*.ts" | grep -v spec`. Якщо лиерал=значення константи знайдено → backend має inline копію.
3. Альтернатива: backend FSM-файл (`apps/api/src/modules/<entity>/<entity>.fsm.ts`) має експортити свою симетричну константу що backend service імпортує. Cross-check: `grep -rn "<SHARED_NAME>" apps/api/src` ⊆ `grep -rn "<FSM_LOCAL_NAME>" apps/api/src` — порушення симетрії = bug.
4. Якщо backend service використовує приватну `static readonly`-константу у класі (як `WorkOrdersService.SHAREABLE_STATUSES` до Bug #432-фіксу) — теж drift, треба експортити у fsm.ts.

**Підхід до фіксу:**

1. Додати backend константу у `apps/api/src/modules/<entity>/<entity>.fsm.ts` (або відповідний `*.constants.ts`). Імʼя — точно дзеркало shared, але без `<DOMAIN>_` префіксу: `INVOICEABLE_STATUSES`, `SHAREABLE_STATUSES`.
2. У кожному service-файлі замінити inline `if (!['X', 'Y'].includes(s.status))` на `if (!CONSTANT.includes(s.status as <PrismaEnum>))`.
3. Видалити приватні `static readonly X = [...]` усередині класів — перенести у fsm.ts.
4. Парний FSM-spec має містити cross-equality тест: `expect(BE_INVOICEABLE_STATUSES.sort()).toEqual([...WO_INVOICEABLE_STATUSES].sort())` — гарантує що drift у майбутньому ламає CI.

**Severity:** HIGH — silent FE-BE divergence при додаванні нового статусу у whitelist. Frontend gate проходить, backend 400. CRITICAL якщо whitelist гейтить financial operations.

**Де шукати ще:** усі модулі з FSM-логікою (PO, Invoice, StockDocument, CompletionAct, Calendar slots). Усі shared `*_STATUSES`, `*_LABELS`, `*_TRANSITIONS` константи. Парний шаблон з Bug #401 (FE asymmetric з backend), але глибше: тут shared CONST = false-friend, бо існує тільки на FE-stage.

---

### 2026-06-11 — Audit-track list incomplete для new fields у update().data (Bug #433) — backend / audit / compliance gap (same family as Bug #421)

**Сигнал:** service `update()` метод має парний `trackField(key)` loop де audit-track keys вказані як `const [...] as const` array. Кількість keys у array < кількість fields у `prisma.<model>.update({ data: { ...fields } })` payload. Будь-який нещодавній commit що додав нове поле у `data` — НЕ оновив парний `trackField` array. AuditEvent.diff для змін цього поля силенто порожній.

**Причина виникнення:** додаючи нове поле у `update()` (наприклад `documentDate`, `liftId`, `plannedHours`), розробник копіює існуючий шаблон data-spread (`<field>: dto.<field>`) і забуває паралельний trackField-array. tsc green бо `trackField` приймає `keyof UpdateXDto` — нове поле теж valid key. Якщо нещодавно вже виправляли той самий шаблон (Bug #421 для plannedHours/actualHours) — high probability існують й інші не-виправлені поля у тому ж списку (документ-дата, FK-and). Сходить за «split-fix»: original review-fix накладено вибірково, інші analogous-fields пропущені.

**Підхід до виявлення:**

```bash
# Знайти усі update() з audit-track list — і diff проти actual data payload
for svc in apps/api/src/modules/*/[a-z]*.service.ts; do
  # знайти data: { ... } всередині update() і audit-track array
  grep -B2 -A50 "prisma\.[a-z]*\.update({" "$svc" | grep -E "^\s+(\w+):" | grep -v "where\|data\|include\|select" > /tmp/data-fields
  grep -B5 -A30 "trackField" "$svc" | grep -E "^\s+'(\w+)'," > /tmp/audit-fields
  # ручний diff: data-fields - audit-fields > 0 → bug
done
```

Empirical: коли коммит має message `fix(review): ... + audit diff` або `fix(tester): Bug #N audit gap`, ВСЯ сімʼя fields у тому ж update() підозріла — не лише той що згадано у commit. Перевірити full data-payload проти full audit-list.

**Підхід до фіксу:** додати пропущене поле у trackField array. Перевіряти що TypeScript `as const` keep array.literal type для type-safe `keyof UpdateXDto` constraint. Якщо новий field має transform у data (`new Date(dto.documentDate)`) — у trackField не міняти, бо oldData використовує raw `wo.documentDate` (Date), newData — `dto.documentDate` (string); diff string-vs-Date OK для JSON serialization у AuditEvent.

**Регресія-guard:** spec кейс `it('update() з { documentDate: '2026-06-15' } викликає audit.record з diff включаючи documentDate')`.

**Severity:** HIGH — compliance/audit-trail gap, особливо для financial/legal data (документ-дата заднім числом, зміна FK).

**Де шукати ще:** усі `update()` методи у `apps/api/src/modules/*/[a-z]*.service.ts` що мають `trackField`/`auditService.record` поряд з `prisma.X.update`. Pair-check для кожної: data-keys ⊇ audit-keys. Профілактично запустити audit після КОЖНОГО `feat(api): add <field>` commit'у у service з audit.

---

### 2026-06-11 — Local TypeScript interface для backend DTO випускає нове поле (Bug #434) — sync / type-drift / silent inline-edit loss

**Сигнал:** компонент у `apps/web/src/components/ui/<Modal>.tsx` оголошує локальний interface `<ResourceDetail>` що дзеркалить backend `<Resource>ResponseDto`. Backend нещодавно додав поле (наприклад `unitOfMeasureId`, `coefficient`, `plannedHours`), оновив `toDto`-mapper, але FE-interface його ВТРАТИВ. TS-компіляція зелена бо локальний interface ніяк не повʼязаний з shared/backend DTO. Споживач (load mapper) хардкодить дефолт (`unitOfMeasureId: ''`) бо не може прочитати поле з типу.

**Причина виникнення:** Sprint що додає нове поле проходить через backend (Prisma → DTO → service → toDto) і потім реалізує FE-консьюмера, який ВИКОРИСТОВУЄ нове поле у display (`unitShortName` для рендера) — але **load mapper** (що копіює response у local form-state) пропускає нове поле через те що локальний interface його не оголошує. Drift: backend response містить поле, FE його ігнорує. Display works (бо backend повертає вже-форматований `unitShortName`), але inline-edit/clone/duplicate path губить раніше-обраний FK.

**Підхід до виявлення:** для **кожного** локального TS interface у `apps/web/src/components/ui/*.tsx`/`apps/web/src/app/**/*.tsx` що по структурі схожий на backend DTO (іменування полів, optional FK з `?: string`) — cross-read відповідний backend `*.dto.ts`. Якщо backend поле X є у DTO але не у FE-interface → silent drift. Альтернатива: винести у `packages/shared/src/types.ts` як shared interface і імпортувати в обох сторонах (long-term fix; коротко — додати field у local interface).

**Підхід до фіксу:**

1. Додати пропущене поле у local interface з тим самим типом, що у backend DTO (`unitOfMeasureId?: string | null`).
2. Оновити load-mapper щоб реально читати з response: `unitOfMeasureId: p.unitOfMeasureId ?? ''`.
3. Перевірити inline-edit handler: чи копіює воно поле у edit-state? (`setEditingPart(p => ({ ...p, unitOfMeasureId: part.unitOfMeasureId }))`)
4. Регресія-guard: компонент-тест `it('edit existing part зберігає UoM з backend')`.

**Severity:** MEDIUM — display-only сценарій працює, drift проявляється у inline-edit/clone. HIGH якщо submit-flow безпосередньо використовує захардкоджений default замість preserved value (data corruption у новій версії entity).

**Де шукати ще:** усі великі modal-компоненти з locally-typed DTO (`CreateXModal`, `EditXModal`, `XDetailModal`). Particularly modal-and поля з complex relations (UoM, contract, lift, employee). Парне з Bug #232 (mass DTO field migration — include audit) та Bug #434 (interface audit).

---

### 2026-06-11 — Dead JSX `<></>` Fragment leftover після refactor "IIFE → inline JSX" (Bug #431) — frontend / code-cleanliness / refactor-followup

**Сигнал:** commit що містить keyword `refactor` АБО `simplify` АБО `perf` у заголовку І чіпає `.tsx` файл-з-IIFE (grep `\(\(\) => \{` в історії файлу до commit'а, або `(\(\) =>` у попередньому commit). Після commit — у тілі компонента залишається `<>` ... `</>` Fragment що НЕ повертається з callback/умовного рендеру, а просто стоїть як inline-child батьківського JSX-element'а. Батьківський element ВЖЕ приймає кілька children (немає одно-children обмеження). Fragment без `key` prop, без conditional-wrap логіки. ESLint правило `react/jsx-no-useless-fragment` зазвичай вимкнене (не у default-конфігах Next.js).

**Причина виникнення:** IIFE-шаблон `{(() => { ... return <>...</>; })()}` у JSX **ЗОБОВ'ЯЗАНИЙ** повертати єдиний JSX-вузол (правило JSX expression). Тому розробник обгортав 2+ child elements у `<>...</>`. Refactor "IIFE → useMemo / inline JSX" видаляє зовнішню обгортку IIFE, АЛЕ забуває видалити внутрішній Fragment бо: (а) механічно знімаючи `(() => { return ... })()` оточення лишається `<>...</>` як «безпечний» wrapper, (б) refactor PR проходить review через `tsc green + tests green` — Fragment не змінює поведінки, (в) lint не warn'ить.

**Підхід до виявлення:** після кожного refactor commit з кейвордами `(refactor|simplify|perf)` що чіпає `.tsx` файл — grep:

```bash
# Знайти Fragment без атрибутів у тілі компонента (не на top-level return)
grep -nE '^\s+<>\s*$|^\s+</>\s*$' apps/web/src/**/*.tsx
```

Кожен match → cross-read контекст (5 рядків вище). Якщо батьківський JSX-element приймає кілька children (вже містить sibling-вузли у тому самому tag) → Fragment dead. Якщо Fragment безпосередньо у return-statement компонента (`return <>...</>;`) — НЕ dead (заміна root для multi-child component без extra wrapper). Якщо Fragment у `condition && (<>...</>)` — НЕ dead (групує дітей для conditional render).

**Підхід до фіксу:** видалити рядки з `<>` і `</>`, перенести 1 tab indent для children. Без зміни логіки. Verify: `pnpm vitest <component>` зелений, `tsc --noEmit` зелений, ESLint clean. Якщо проект має lint-rule `react/jsx-no-useless-fragment` — оптимально enable і знайти ВСІ випадки одним проходом (часто 5-10 leftover Fragments по всьому codebase).

**Severity:** LOW (cosmetic, no runtime impact). Не release-blocker, але **накопичується**: десятки dead Fragments → reconciliation overhead + cognitive load у читанні коду.

**Де шукати ще:** після refactor commits у `CreateWorkOrderModal.tsx`, `CalendarSlotModal.tsx`, інших великих modal/page компонентах де часто застосовуються IIFE для inline computed renders. Парний антипатерн: `condition ? <>...</> : null` коли той самий ефект досягається через `condition && <single-child/>` або conditional-wrap у `condition && (<div>...</div>)`.
...
**Сигнал:** Service-метод з ім'ям `refreshFromX` / `syncFromY` / `importFromZ` / `recalculateW` / `regenerateV` / `rebuildU` / `applyFromQ` — який мутує той самий resource (Invoice/PurchaseOrder/StockDocument) що і канонічний `update()`/`addLine()`/`removeLine()`, АЛЕ зі своїм окремим контрактом (зазвичай "перезаписати все з джерела"). `delete deleteMany + createMany` patterns у такому методі — особливо підозрілий. Канонічний `update()` має `if (X.status !== 'DRAFT') throw BadRequestException`; alternate endpoint обходить це.

**Реальний приклад (Bug #403):** `InvoicesService.refreshFromWorkOrder()` шукав existing через `status: { not: 'CANCELLED' }` — пропускав 4 інші активні статуси (DRAFT, SENT, PAID, OVERDUE). У межах `$transaction` робив `deleteMany invoiceLine` + `createMany` з нових даних, потім `recalcTotals` оновлював `Invoice.amount`. PAID-рахунок з реальним `Payment` записом отримував перезапис рядків і `amount` без жодної помилки → бухгалтерська невідповідність (Payment.amount != Invoice.amount). Canonical `update()` має guard `if (inv.status !== 'DRAFT') throw BadRequestException('Редагувати можна лише чернетку')` — refresh цей guard ігнорував.

**Причина виникнення:** Розробник додав alternate endpoint для нової фічі ("оновити рахунок з даних наряду після правок") і не виконав перевірку повноти guards. У свідомості: "це окремий потік, статус-перевірка зробить FSM на парній transition" — забуваючи що ця refresh-операція не йде через transition, а пряма мутація. Pattern також зустрічається з `bulkUpdate` / `applyToAll` endpoints що приймають фільтр і пишуть на множинні рядки.

**Підхід до виявлення:**

1. Знайти всі alternate mutation methods у services:

   ```bash
   grep -rnE "async (refresh|sync|import|recalculate|regenerate|rebuild|apply|reset|cleanup)[A-Z]" apps/api/src/modules --include="*.service.ts"
   ```

2. Для кожного знайденого методу — у тому ж файлі знайти канонічний `update()` / `updateLine()` / `updateX()`. Витягти ВСІ `if (...) throw` guards (особливо `existing.status !== 'DRAFT'`, `existing.isLocked`, `existing.isSystem`, paid-record checks).

3. Перевірити що alternate-метод має ті самі guards (можливо у іншому формулюванні, але семантично еквівалентні).

4. Парне: будь-який mutation що приймає `parentId` і робить `deleteMany child + createMany child` (full overwrite child resources) — обов'язково prep-check status батьківського resource через `if (parent.status !== <ALLOWED>) throw`. `findFirst` parent з `status: { not: CANCELLED }` — НЕ guard, це фільтр пошуку, не business check.

**Підхід до фіксу:** Додати prep-guard після `if (!existing) throw NotFound`:

```ts
if (existing.status !== InvoiceStatus.DRAFT)
  throw new BadRequestException('Оновити можна лише чернетку. Скасуйте поточний і виставте новий.');
```

Регресія-guard: contract spec для alternate endpoint що мокає existing.status=non-DRAFT → 400. Без такого тесту наступний refactor видалить guard і CI пройде зеленим.

**Severity:** CRITICAL (фінансовий ризик для invoice/payment/settlement resources; HIGH для inventory).

**Де шукати ще:**

- `InvoicesService.refreshFromWorkOrder` — fixed Bug #403
- Будь-які майбутні `refreshFromQuote` / `syncFromSubscription` / `rebuildFromHistory` методи
- `PurchaseOrderService.applyPricingChanges()` — pattern bulkUpdate на lines
- `StockDocumentService.recalculateBatches()` — pattern recalc на closed document
