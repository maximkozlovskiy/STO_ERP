...
**Сигнал:** commit `refactor(...extract)`/`винести ... у <NewService>` що переносить методи ДОСЛІВНО у новий сервіс, а review підтвердив byte-identical. Пастка: happy-path тести часто переносяться/лишаються зеленими (викликають ті самі методи), АЛЕ дрібні захисні гілки всередині перенесеного методу тихо лишаються БЕЗ прямого тесту — бо їх ніколи й не було, або вони жили в іншому describe що не переїхав. Тут два класи: (1) **edge-throw** (`if (chargeAmount <= 0) throw` на COMPLETED) — жодного тесту, хоча return-дзеркало мало zero-total-тест; (2) **division-by-zero fallback** (`safeCoeff(coeff)` проти coeff=0 з legacy/seed) — конверсія тестувалась лише з coeff≠0, самий fallback (0/NaN/undefined→1) — ні. Плюс окремий клас: **tenant/guard-фікс усередині перенесеного private-хелпера** (`goodUoM.findMany` +orgId) — юніт-mock Prisma НЕ виконує guard-extension, тож «фікс перевірено» = лише mock-shape-assert, а не реальний guarded-шлях. Grep для аудиту після extract-коміту: `git show <sha> --stat` → для нового `.service.ts` знайти всі `throw`/`safeCoeff`/`?? `/`/ coeff`/`Number.isFinite` гілки → звірити чи кожна має прямий тест у `<new>.service.spec.ts` (не лише у successіному describe материнського сервісу).
**Причина виникнення:** «екстракція byte-identical + happy-path зелений = покриття збережено» — але покриття міряється по РЯДКАХ що виконались, а edge-throw/fallback-рядки виконуються лише на специфічному вході, якого happy-path-тест не подає. І «tenant-фікс має mutation-verified unit» звучить достатньо, поки не згадати що guard живе в $extends, який мок обходить.
**Підхід до виявлення:** після будь-якого extract-коміту — НЕ довіряти «suite зелений», а перелічити захисні гілки перенесеного коду (throw на невалідному стані; safeCoeff/`?? 1`/clamp; `Number.isFinite`; single-vs-batch null-гілки) і для КОЖНОЇ спитати «який тест червоніє якщо її прибрати?». Для guard/tenant-фіксу всередині — обов'язково ІНТЕГРАЦІЙНИЙ тест проти реального guarded-PrismaClient (дзеркалить `tenant-guard.integration.spec.ts`), бо unit-mock не виконує екстеншн. Найсильніший доказ: revert самого фіксу у джерелі → integration-тест відтворює ОРИГІНАЛЬНИЙ throw наживо (тут: `TenantIsolationError: GoodUoM findMany без tenant-фільтра`).
**Підхід до фіксу:** додати mutation-verified unit на кожну незакриту захисну гілку (edge-throw: невалідний вхід→throw+побічний-ефект НЕ стався; fallback: coeff=0→baseQty скінченне, не Infinity/NaN) + один live-DB integration що ганяє перенесений метод через guarded client із засідженим edge-входом (UoM-частина) і асертить 0 guard-throw, з mutation-baseline (без orgId→throw) поряд. Джерело НЕ чіпати якщо фікс коректний — лише тести.
**Severity:** MEDIUM (сам рефактор коректний; ризик — майбутній «тихий» refactor цих гілок пройде CI; для tenant-фіксу без live-тесту ризик HIGH — регресія orgId відкрила б крос-tenant/500 без падіння юнітів).
**Де шукати ще:** будь-який A-серії God-object split (WorkOrdersService→events/stock-effects/share), витягнуті inventory/settlements-хелпери, майбутні extract у invoices/purchase-orders/stock-documents. Скрізь де перенесений метод має `throw`/`safeCoeff`/nullable-branch АБО робить guarded Prisma-запит у private-хелпері. Парне з Bug #705-#706 (mock-рівнева асерція замість real-invariant) і #429 (stale-mock після extract).
...
**Сигнал:** будь-який fail-closed guard (tenant-isolation `whereHasTenantScope`, RBAC where-injector, soft-delete-фільтр), що вирішує «where несе потрібний токен» за критерієм ПРИСУТНОСТІ ключа (`obj[t] !== undefined` / `t in where` / `!!where[t]`), а не за ФОРМОЮ його значення. Пастка: Prisma-токен може бути не лише скаляром рівності — `{not:X}`, `{notIn:[...]}`, `{gt/gte/lt/lte}`, або під `NOT:{orgId:X}`. Усі проходять «ключ присутній», але СЕМАНТИЧНО матчать ЧУЖІ рядки (негація orgId = всі інші tenant-и) → guard, задуманий гучно кидати на забутий фільтр, тихо його обходить. Тут: `{orgId:{not:X}}`/`{NOT:{orgId}}`/`{orgId:null}` (null≠undefined) повертались `true`; частина-(2) «composite-key scan» ще й трактувала `NOT` як composite-унікальний ключ з orgId. Grep: `grep -rnE "!== undefined|\bin\b .*where|Object\.keys\(where\)" apps/api/src/**/*guard*.ts *tenant*.ts` → для кожного «токен присутній» перевірити чи розрізняє позитивну рівність від негації; звірити чи логічні ключі (AND/OR/NOT) виключені з composite-scan. Живий доказ (mutation): `whereHasTenantScope({orgId:{not:'x'}})` → якщо `true` = leak-вектор.
**Причина виникнення:** «якщо orgId є у where — запит tenant-scoped» — розумно для 99% викликів (скаляр рівності), але Prisma-filter-DSL дозволяє негацію/діапазон на тому ж полі, а логічні комбінатори вкладають об'єкти що структурно схожі на composite-ключі. Присутність ключа ≠ прив'язка рядка до конкретного орендаря.
**Підхід до виявлення:** статично — guard-предикат, що перевіряє існування токена а не його ФОРМУ; будь-який `Object.keys()`-scan вкладених об'єктів без skip AND/OR/NOT. Динамічно — прогнати predicate по матриці негативних форм (`{not}`/`{notIn}`/`{gt}`/`null`/`NOT:{token}`/`{in:[]}`) + позитивних (`scalar`/`{in:[non-empty]}`/`{equals}`); leak якщо хоч одна негативна → true. Обов'язково і live-integration проти реального guarded-client (мокнуті Prisma-юніти НЕ виконують екстеншн).
**Підхід до фіксу:** «позитивна tenant-binding» = лише рівність/обмежена множина: `isPositiveTenantBinding(v)` → scalar true; `{in:[non-empty]}`/`{equals:scalar}` true; `{not}`/`{notIn}`/`{gt/gte/lt/lte}`/`null`/`{in:[]}`/невідома-форма → false (fail-closed default). Composite-key scan пропускає `{AND,OR,NOT}`. Зберегти легіт cross-org `{token:{in:[...]}}` (batch-scheduler). Regression: unit-матриця (leak-форми→throw, positive→pass) mutation-verified (revert → leak-тести червоні) + live-DB integration.
**Severity:** CRITICAL (крос-tenant data-leak через defeat fail-closed guard; латентний якщо жоден поточний виклик не пише негований токен, але один майбутній cross-org report/admin-запит його тригерить).
**Де шукати ще:** усі guard-екстеншни/where-injector-и що судять «scope присутній»: RBAC branch-фільтр, soft-delete-guard, org-scoped-cache-key builder; будь-де де `NOT:{...}`/`{not}` може стосуватись tenant/authz-поля. Парний gap: integration-spec guard-а без delete/deleteMany/groupBy/upsert-throw кейсів (gap #722 — операції у GUARDED_WHERE_OPS без live-покриття).
