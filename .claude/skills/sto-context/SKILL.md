...
**До будь-якої відповіді** виконай наступне:

1. Прочитай файл `docs/PHASES.md` (Read tool або bash `cat`)
2. Знайди **першу задачу з `[ ]`** (незакреслену)
3. Зібери **нотатки** `>` під останніми `[x]` задачами поточної фази — це контекст що вже зроблено
4. Виведи блок статусу:
...
6. Якщо всі задачі фази завершені — перейди до наступної
