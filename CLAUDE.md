...
3. **Оновити `MemoryManual.md`** — обов'язково після кожної сесії змін:
   - Новий компонент / хук / утиліта → розділ "UI / Компоненти"
   - Нова фіча на сторінці → розділ відповідної сторінки або "Фічі"
   - Новий API endpoint або зміна DTO → розділ "API"
   - Новий gotcha / підводний камінь → розділ "Gotcha"
   - Виправлений баг з нетривіальною причиною → розділ "Gotcha"
   - Оновлення `TypeScript:` рядка і `Останній commit`

> Виняток: якщо сам запит був review або tester агент — не запускати рекурсивно.
> MemoryManual оновлюється **завжди**, навіть для дрібних фіксів — хоч одним рядком.
