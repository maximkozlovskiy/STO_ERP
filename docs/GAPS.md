...
| T20 | Backend  | (STALE — звірено в коді) polling auto-recovery ВЖЕ є (MAX_POLL/wall-clock→EXPIRED + finalizePayment reconcile-гілка + P2002-idempotency Bug#688 + MAX_FINALIZE-cap); WO INVOICED→PAID поза tx — BY-DESIGN (WorkOrdersService.transition має власну tx, nested-tx неможливий; payment+settlement+paidAmount уже закомічені, fail→.catch-лог+retry оператором); post-commit side-effects (notify/checkbox/loyalty) поза tx — BY-DESIGN offline-first (усі з .catch, money-op уже committed). Реального боргу немає | Низький  | 🟢     |
...
| A3  | Розбити God-об'єкти: WO-service 2124→1595р (Share✅+Events✅+StockEffects✅); CreateWorkOrderModal 3708→2322р (3 хуки✅+PartsTable✅+WorksTable✅) — обидва God-об'єкти розкладено | Високий  | 🟢     |
