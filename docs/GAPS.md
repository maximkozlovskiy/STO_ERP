...
| T26 | Quality  | E2E-suite частково червоний: estimate-share + work-orders-features (13 тестів) timeout-ять на стейл-селекторах модал-open («Кошторис»/FSM-pills/Документи-tab) — pre-existing seed/selector-дрейф, НЕ регрес від A3-modal (diff не чіпає ці елементи). Блокує E2E як regression-net → потрібен sto-e2e-прохід перед step-3 CreateWorkOrderModal | Низький  | 🔴     |
...
| A3  | Розбити God-об'єкти: WO-service 2124→1595р (Share✅+Events✅+StockEffects✅); CreateWorkOrderModal 3708→3524р (useReferenceData✅+useStockTotals✅; лишаються useWorkOrderActions+таблиці) | Високий  | 🟡     |
