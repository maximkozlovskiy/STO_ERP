...
38baaea2 fix(tester): stock-totals — coverage gaps + Bug #454 useEffect perf
dd3fda07 docs(skills): add 3 patterns to sto-review — colSpan-after-new-col, bulk-ids UUID validation, useEffect race-guard
83cd37f7 docs(memory): update MemoryManual after review c0879445 — stock-totals column fixes
c0879445 fix(review): stock-totals column — tfoot colSpan, race guard, UUID validation
0618c621 feat(work-orders): add K-t na skladi column to parts table
af09a681 fix(tester): Bugs #449, #451 — Bug #448 regression tests + date='' guard
07d6afed docs(skills): add boundary-constant comment drift pattern to sto-review
dd91a138 docs(memory): update MemoryManual after review 02e16389 — stale comments fix
02e16389 fix(review): stale comments — WINDOW_END is 20:00, not 19:00
94de0b34 fix(calendar): correct WO prefill from calendar slot (Bug #448)
294a5ed6 docs(skills): add 2 perf patterns to sto-optimize after dashboard/calendar audit
747cc445 perf(optimize): dashboard kyivToday hoist + calendar sync spec sync
...
68cbf93c docs(memory): update MemoryManual after review 1d9cd19d — calendar contract spec + dropped counterpartyId SELECT
...
Latest tester (2026-06-15, after 821de2d2): /sto-tester audit для PurchaseOrder edit-mode + RECEIPT type — 2 MEDIUM bugs (test-coverage gaps для FSM transition), all fixed.
  ...
  - Перевірено end-to-end: tenant isolation у update() OK (Bugs #473-#476 покрити), contractId nullable у DTO @ValidateIf OK (Bug #477 покрито), stock-documents tab bar містить RECEIPT (рядок 337), StockDocumentsService.transition(RECEIPT→CONFIRMED) покрито Bug #480.
  - Tests: API 797 → 820 (+23 PO FSM transition), web 423 unchanged. TS clean.
Previous tester (2026-06-15, f59c6a47, after a067ec21): /sto-tester audit feat(stock-documents) RECEIPT — 3 HIGH bugs (test-coverage gaps), all fixed.
...
Latest tester (2026-06-15, after dae793c8): tester run перевірив 3 переписані модалки (Invoice/PO/StockDoc) і list-pages інтеграцію. 8 bugs знайдено і виправлено:
  ...
  🐛 #460 (CRITICAL): PurchaseOrderCreateModal.handleCreate робив POST /purchase-orders/:id/lines (endpoint НЕ існує) — 404 на кожен виклик, PO залишався orphan-draft без lines. Виправлено: `lines: [...]` тепер передаються у body POST /purchase-orders (backend service.create вже атомарно у $transaction).
  🐛 #461 (HIGH): InvoiceCreateModal.handleSave не видаляв існуючі рядки що користувач прибрав через UI. Додано initialLineIdsRef snapshot + DELETE /invoices/:id/lines/:lineId для removedIds.
  🐛 #462 (HIGH): StockDocumentCreateModal не валідувала targetWarehouseId для TRANSFER — кнопка enabled, POST повертав 400. Додано disabled condition + inline error у handleCreate.
  🐛 #463 (MEDIUM): InvoiceCreateModal handleCreate надсилав amount=0.01 placeholder. Тепер computedTotal = sum(qty*unitPrice).
  🐛 #464 (MEDIUM): stock-documents/page.tsx мав dead imports useRef/useEffect — видалено.
  🐛 #465 (MEDIUM): stock-documents/page.tsx — `const load = invalidate` оголошено ПІСЛЯ useCallback що його використовує. Перенесено перед.
  🐛 #466 (HIGH): жодного component test для 3 нових модалок (3000 рядків коду). Створено `DocumentCreateModals.test.tsx` з 3 regression-тестами (по одному на bug #460, #461, #462).
...
BUG CYCLE (3 bugs fixed auto):
  ...
  🐛 #453 (tester): GET /goods/stock-totals без contract spec — новий `goods-stock-totals.contract.spec.ts` з 12 HTTP кейсами (missing/empty ids, valid, UUID validation, SQL-inj guard, limit-100, boundary, dedup, case, malformed, RBAC).
  🐛 #454 (tester): useEffect refetch на кожну keystroke — fixed: `useMemo(sortedGoodIds)` key замість deps `[parts, newPart.goodId, editingPart.goodId]` → скорочено RTT з ~100 keystroke-triggered fetches → 1 actual.

Latest review (2026-06-14, 4a05d7c7, after fd8be3b3 perf cycle + uncommitted byWarehouse feature): full QA across inventory 3-view stack + CreateWorkOrderModal per-warehouse stock. 5 fixes — (1) CRITICAL §2.1 pricing leak: `GET /stock-items/by-batch` мав `@Roles(..., 'MECHANIC')` але response повертає `batch.costPrice` та `batch.salePrice` (inventory.service.ts:527-528); вилучено MECHANIC — pricing-доступ обмежений OWNER/ADMIN/STOREKEEPER/RECEPTIONIST. (2) IMPORTANT type-drift: `ConsumptionRow` в `byBatch()` був типізований як `documentType: string | null` / `documentId: string | null` тоді як schema-фактично NON-null (BatchConsumption :1212-1213) і frontend `BatchConsumptionRow` теж non-null; tightened backend type щоб запобігти dead null-checks і drift. (3) SUGGESTION §1: `goods.service.stockTotals()` per-warehouse `findMany` не мав `take` cap; додано `take: 2000` (100 goodIds × ≤10 warehouses = ~1000 typical, defensive ceiling). (4) FEATURE state-reset: `CreateWorkOrderModal` early-return при пустому `stockGoodIdsKey` скидав `stockTotalsMap` але НЕ новий `stockWarehouseMap` — stale per-warehouse data persisted при clear-форми; додано симетричний `setStockWarehouseMap(new Map())`. (5) FEATURE shipping: `GET /goods/stock-totals` тепер повертає `{ goodId, totalQuantity, byWarehouse: [{ warehouseId, quantity }] }`; "К-т на складі" колонка в CreateWorkOrderModal показує `{whQty} / {total}` (specific-warehouse / cross-warehouse) замість лише total. TS clean. Tenant isolation OK (orgId у where скрізь), routes ordered, OPENING_BALANCE присутній у MOVEMENT_TYPE_LABELS.

Latest review (2026-06-14, 1752a75, after a5f01d37 3-view stock report): 4 фікси у `inventory/page.tsx` + `inventory.service.ts` — (1) CRITICAL: shorthand JSX fragments `<>...</>` всередині `.map()` в `ByDocumentsView` і `ByBatchesView` — React не приймає `key` на `<>`, тож `key={good.goodId}`/`key={bg.batchGroupKey}` на внутрішніх `<TableRow>` НЕ задовольняли keyed-list контракт для іterованого елементу (re-render/order bugs + console warning); замінено всі 4 пари (good loop, doc inner loop, batch group loop, good-in-batch inner loop) на `<Fragment key={...}>` з імпортом `Fragment` з 'react'; (2) IMPORTANT: `MOVEMENT_TYPE_LABELS` мав ключ `ADJUSTMENT` якого немає в `StockMovementType` enum, і пропускав `OPENING_BALANCE` що ЄСТЬ в enum — синхронізовано з schema.prisma; (3) SUGGESTION/perf: `kyivOffsetMs()` створював `new Intl.DateTimeFormat()` на кожен виклик (2× per byDocument/byBatch request); hoist до module-level `KYIV_HOUR_FMT` singleton; (4) IMPORTANT defense-in-depth: `byDocument`/`byBatch` queries не фільтрували рухи/батчі для soft-deleted goods/warehouses — додано `good: { deletedAt: null }` + `warehouse: { deletedAt: null }` filters (StockMovement і StockBatch не мають власного deletedAt, тож relation-side filter — єдиний guard). TS clean. Tenant isolation OK (orgId скрізь), routes ordered: `/by-document`+`/by-batch` ПЕРЕД `:id` у controller (Fastify declaration-order).

Latest tester (2026-06-14, 38baaea2, after review cycle 0618c621→c0879445): Bugs #452 (coverage), #453 (coverage), #454 (perf). Додано 19 unit-тестів для stockTotals (7 service + 12 contract) + 4 regression-guard тести в CreateWorkOrderModal.test для stockTotalsMap deps. Виправлено: useEffect refetch на кожен keystroke → stable `useMemo(goodIdsFingerprint)` key; goods без StockItem маплять на `0`; UUID validation в controller; tfoot colSpan bumped. TS clean. API 744 OK, web 406 OK.

Latest review (2026-06-14, c0879445, after 0618c621 stock-totals column): 3 фікси у `CreateWorkOrderModal.tsx` + `goods.controller.ts` — (1) CRITICAL layout: tfoot "Разом товарів:" `colSpan` не був інкрементнутий після додавання нової колонки "На складі" → totals (ПДВ/сума) виштовхувались у колонку дій; bumped `4→5` (VAT) і `5→6` (no VAT) щоб відповідати новій layout 9/8 cells; (2) IMPORTANT race-safety: `useEffect` що тягне `/goods/stock-totals` не мав cancelled-guard → stale-response race коли `parts`/`newPart.goodId`/`editingPart.goodId` швидко змінюються; додано `let cancelled = false` + cleanup; також goods без StockItem rows (`groupBy` пропускає порожні бакети) тепер маплять у `0` замість `'—'` для коректного UX; silent `.catch(() => {})` → `console.error` (по convention §8.2); (3) IMPORTANT §2.3 input validation: `GET /goods/stock-totals?ids=...` не валідував UUID → invalid value падав у Postgres як 500 "invalid input syntax for type uuid" замість 400; додано модульний `UUID_RE` regex + early `BadRequestException`; `@Query('ids')` зроблено optional (`ids?: string`) щоб відповідати тілу handler-а яке вже обробляє відсутній параметр.
Latest review (2026-06-14, 02e16389, after 94de0b34 Bug #448 fix): 1 фікс (3 stale-comment місць) у `CalendarSlotModal.tsx` — після виправлення `isOverflow` на `WINDOW_END * 60` (де `WINDOW_END = 20`) інші коментарі досі посилались на старі значення: (1) `calcEndAt()` jsdoc "(19:00 = 1140 min)" → "(20:00 = 1200 min)"; (2) `addSlot()` "e.g. 21:00 for 2 h from 19:00" → "22:00 for 2 h from 20:00"; (3) внутрішній comment "slot2 duration = endAt - 19:00 (WORK_DAY_END_H). E.g. 2h from 19:00 → endAt=21:00" → "endAt - 20:00 ... endAt=22:00". Pattern: повторний "calendar comment drift" (як у fef027b0) — при зміні boundary-константи треба grep по всіх посиланнях на старе значення. TS 0 errors. Frontend WINDOW_END = backend WORK_DAY_END_H = 20.
...
Latest tester (2026-06-14, 38baaea2, after c0879445 stock-totals review): Bugs #452 (MEDIUM), #453 (MEDIUM), #454 (MEDIUM) — coverage gaps + perf. (1) #452 додано 7 unit tests у `goods.service.spec.ts` для `stockTotals()` — порожній ids, orgId+deletedAt where, multi-warehouse SUM, missing StockItem (groupBy опускає), `_sum.quantity=null → 0`, cross-tenant isolation, soft-delete виключення. (2) #453 створено новий contract spec `goods-stock-totals.contract.spec.ts` з 12 кейсами на HTTP-рівні: missing/empty ids, valid UUIDs, trim + порожні token-и, invalid UUID → 400, SQL-injection → 400, >100 ліміт, рівно 100 boundary, дублі (client dedup responsibility), upper-case UUID, malformed UUID, 403 RBAC. (3) #454 frontend perf — useEffect deps `[parts, ...]` re-fired на кожен keystroke у quantity/price → spurious GET /goods/stock-totals; виправлено через `useMemo(stockGoodIdsKey)` (sorted-comma-joined fingerprint set-у goodId-ів) + deps `[stockGoodIdsKey]`. Sort забезпечує що reorder parts не дає false-positive refetch. +4 frontend tests (mount без goodId → no fetch, edit-mode load → fetch з goodIds, error console.error не silent, dedup через Set, stable sort order). Tests: API 744/744 (+19), web 406/406 (+4). TS green.
Latest tester (2026-06-14, af09a681, after 94de0b34 Bug #448 fix): Bugs #448 (HIGH, recorded retroactively), #449 (MEDIUM), #451 (LOW) — (1) #448 audit-trail запис для commit 94de0b34 що виправив hardcoded boundary `19 * 60` (правильно `WINDOW_END * 60 = 20 * 60`) у `CalendarSlotModal.tsx:1464` + відсутній `plannedHours` у `CreateWOPrefill` інтерфейсі/return; (2) #449 — додано 3 regression тести у `CreateWorkOrderModal.test.tsx`: prefill.plannedHours використовується напряму, overflow slot з next-day endAt не дає ~15.5h ghost, backward compat fallback на calcPlannedHours; (3) #451 LOW — defensive guard `form.startAt && date` у prefill plannedStartAt/plannedEndAt build (захист від `date = ''` на першому render до URL default effect). Web tests: 398 → 401. API tsc + web tsc green. Pattern: будь-який data-flow fix між компонентами (інтерфейс додано/змінено + fallback логіка) — обов'язково парний regression-guard навіть якщо feature виглядає тривіальною; інакше silent regression при refactor.
...
Будь-який pair `BE const X` + `FE const WO_X` (або analogous prefix-mirror у `@sto/shared`) з коментарем «Must mirror …» / «Mirrors …» — це **інтенція**, не invariant. Без `expect(BE.sort()).toEqual([...FE].sort())` у `*.fsm.spec.ts` майбутній developer може додати стан у одну сторону, інший бік мовчки розходиться, tsc + unit зелені (бо BE-тести читають BE, FE-тести читають FE), runtime UX gap (canEdit=false коли backend дозволив). Перевірка коштує 5 рядків spec, ловить CRITICAL regression. Pattern: при додаванні shared status-сету одразу писати symmetry-spec у BE invariants file (де вже є fsm-тести).
