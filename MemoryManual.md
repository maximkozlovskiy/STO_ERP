...
Latest optimize: 2026-08-30 (FULL /sto-optimize, HEAD a5fc685e, feat/supplier-payments, цикл 1/3) — 3 знайдено, 3 виправлено. (1) DB index counterparty_contracts(orgId, contractType, deletedAt) — getSchedule фільтрував за (orgId, contractType='PURCHASE') БЕЗ counterpartyId, існуючий (orgId, counterpartyId, deletedAt) не покривав → heap-scan усіх контрактів на кожен polling (staleTime=30s). (2) DB index supplier_payments(orgId, purchaseOrderId, deletedAt) — покриває findAll(purchaseOrderId=?) + getSchedule.groupBy(purchaseOrderId=null) + Prisma nested include supplierPayments у RECEIVED/PARTIAL POs. (3) purchase-orders.service.receive(): Array.find у map → Map bucket-by-id (O(N×M)→O(N+M), 10_000→200 порівнянь на PO зі 100 ліній). Нова migration 20260830120000_add_schedule_hotpath_indexes.sql. SKILL.md +2 нових patterns («aggregation-endpoint reference discriminator drift», «reverse-FK axis miss child-table»). TS: api ✅ 0 | web ✅ 0. Vitest: 992/992 + 488/488.
...
Останній commit: 2026-08-30 — a5fc685e perf(optimize): schedule hot-path indexes + bucket-by-id line lookup у PO.receive (migration 20260830120000 + purchase-orders.service). Попередній: d5d58af7 fix(tester): Bugs #592-#595 (DST-aware spec + QCProvider wrap + SP create hook migration + strict date DTO). Baseline було червоне (API 991/992 + Web 484/488), після коміту зелене (992/992 + 488/488). Попередній: 2026-06-20 — fix(tester): фінальна верифікація 300 E2E тестів. Виправлено 4 failed:
