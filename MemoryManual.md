...
Latest review: 2026-06-11 (HEAD 86d22367 — security follow-up на 9d87c119 strip-PII commit, scope: checkConflicts response):
  • SUGGESTION (fixed) — попередній commit 9d87c119 видалив `vehicle`/`counterparty`/`workOrder.counterparty`/`workOrder.vehicle` joins з `CONFLICT_SELECT`, але лишив поле `counterpartyId` у select + `toConflictDto` повертає `slot.counterpartyId ?? null`. Жоден консумент (CreateWorkOrderModal + CalendarSlotModal) не читає `conflict.conflictSlots[i].counterpartyId` — використовуються лише `anyConflict`/`liftConflict`/`employeeConflict`/`conflictSlots.length`/`conflictWoNumbers`. Defense-in-depth: MECHANIC з доступом до `/calendar/slots/check-conflicts` може за допомогою time-window probing зчитати UUIDs клієнтів через кросс-WO конфлікти → потенційно useful для подальших атак enumeration. Фікс: видалено `counterpartyId: true` з CONFLICT_SELECT, видалено `counterpartyId` поле з типу параметра `toConflictDto`, hard-code `counterpartyId: null` у return. Frontend `ConflictSlot` interface теж очищено.
  • Перевірено: TypeScript 0 errors (api + web + shared), без регресій у консумерах (jsx grep `conflict\.counterpartyId` — 0 збігів). Інших leak-полів у toConflictDto немає: vehicleId/notes/counterpartyName/cpPhone/vehicleSummary/vehiclePlate усі hard-coded null.
  • Out-of-scope (flagged, не виправлено у цьому review): pre-existing `findSlots` GET endpoint (`/calendar/slots`) повертає `phone`/`licensePlate`/`firstName`/`lastName`/`companyName` для роль MECHANIC через `findSlots(@Roles('OWNER','ADMIN','RECEPTIONIST','MECHANIC'))` + повний `SLOT_INCLUDE` у toDto. Це ширший security gap, але lежить поза diff scope поточного commit. Кандидат на окрему фічу: 2-варіантний toDto (full / mechanic-mask) або per-role projection через декоратор.
